 ...
 */
/*int logger(const string reason, log_info info) {
	FILE *fp; 
	...
	//compiler problems so I commented out.  Will look at later
 ...
 */
/*int log_no_entry_found(string destIP, string destPort, string srcIP, string srcPort, uint8_t priority, uint32_t length) {
	struct hostent *he;
	struct in_addr ipv4addr;
	char srcName[32], destName[32];
	
	// Get the destination hostname
	inet_pton(AF_INET, destIP.c_str(), &ipv4addr);
	he = gethostbyaddr(&ipv4addr, sizeof ipv4addr, AF_INET);
	strcpy(destName, he->h_name);
	
	// Get the source hostname
	inet_pton(AF_INET, srcIP.c_str(), &ipv4addr);
	he = gethostbyaddr(&ipv4addr, sizeof ipv4addr, AF_INET);
	strcpy(srcName, he->h_name);
	
	// Fill out the log structure and call the logger function.
	log_info info;
	strcpy(info.src_hostname, srcName.c_str());
	strcpy(info.src_port, srcPort.c_str());
	strcpy(info.destination_hostname, destName);
	strcpy(info.destination_port, destPort.c_str());
	ftime(&info.time);
 ...
 */
/*int log_entry(string log_message, packet pkt) {
	struct hostent *he;
	struct in_addr ipv4addr;
	char srcName[32], destName[32];
...
void queue_packet(packet pkt, int index) {
	...
	pkt_plus = (packet_plus*)malloc(sizeof(packet_plus));
...
bool dealWithDelay(int socketFD_Emulator) {
	...
	if (end_delay) {
		...
		if (drop && delayed_pkt.pkt.type != 'R' && delayed_pkt.pkt.type != 'E') {
			...
			char* sendPkt = (char*)malloc(P2_HEADERSIZE+HEADERSIZE+delayed_pkt.pkt.length);
